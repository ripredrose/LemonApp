    ...
    <section className='testimonials'>
        ...
        <section className='opinions '>
            {reviews_info.map((review)=>{
              return  <article className='opinion'>
                        <div className='rating'>
                          {Array.from(Array(review.stars)).map((_) => <img src="/assets/star" alt="star" className='star'/>)}
                        </div>
                        <div className='customer-info'>
                            <img src={"/assets/"+review.name} alt='customer_image' className='profile'></img>
                            <h5>{review.name}</h5>
                        </div>
                        <div className='highlight-text review-text'>{review.opinion}</div>
                      </article>
            })
